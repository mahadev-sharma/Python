...
"""
import random
import simplegui#this does not support by
...
def draw(canvas):
    try:
        # Draw choices
        canvas.draw_text("MAHADEV~ " + human_choice, [10,80], 48, "blue")
        canvas.draw_text("RIVAL ~ " + computer_choice, [10,160], 48, "green")
        
        # Draw scores
        canvas.draw_text("MAHADEV~ " + str(HUMAN_SCORE), [800,300], 30, "blue")
        canvas.draw_text("RIVAL ~ " + str(COMPUTER_SCORE), [800,360], 30, "green")
...
def play_rps():
    frame = simplegui.create_frame("WELCOME TO MAHADEV'S WORLD", 1088, 650)
